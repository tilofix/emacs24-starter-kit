...
Update [2022-07-08 Fr] : after I moved my "workspace" 
from labtop 56S8TZ1 to laptop 2PP4QN3 I ran again in the 
tool-bar and menu-bar issue. Thanks to this note I got rid of both
by calling =M+x org-babel-tangle= (after saving this line, need to exec again :-).
...
#+begin_src emacs-lisp 
  ...
  (when (getenv "ORG_HOME")
    (let (
          (org-info-dir
           (expand-file-name "doc" (getenv "ORG_HOME"))))
      (when (file-directory-p org-info-dir)
        (setenv "INFOPATH" (concat org-info-dir ":" (getenv "INFOPATH")))
        )))
#+end_src
...
Just learnt now after so many years [2020-04-17 Fr],
let's use start up [[file:starter-kit-python.org][python]] from optional starter kit packages.
#+BEGIN_SRC emacs-lisp :tangle yes
...
[2020-04-17 Fr] After introducing starter-kit's python I've got following error message
'inferior-python-mode: Wrong type argument: stringp, nil'
and maybe also after Emacs has been upgraded 
to 'GNU Emacs 26.3 (build 1, x86_64-pc-cygwin) of 2019-08-30'
but the (I)Python shell is running, but has got a side effect
#+BEGIN_EXAMPLE
  In [2]: [?25
  In [2]: 
...
#+begin_src emacs-lisp :tangle yes
  ...
  (if (eq system-type 'cygwin)
      (custom-set-variables
       ...
       ; Set python-shell-completion to avoid warning
...
** IPython
IPython is installed via emacs' package manager.
...
and has [[https://nbviewer.jupyter.org/][notebook viewer]] based on GitHub.
  ...
  (setq org-capture-templates
        '(("n" "Note" entry
          ...
          ("p" "Personal Note" entry
           (file+headline "~/Documents/Wiki/worg/notes_personal.org" "Personal Notes")
           "* %?\n %u")
          )
        )
#+end_src
...
#+begin_src emacs-lisp :tangle yes
  ...
  (setq plantuml-jar-path
        (expand-file-name "~/Library/plantuml.8030.jar"))
...
#+begin_src emacs-lisp :tangle yes
  ...
  (add-to-list 'org-link-abbrev-alist '("rediscmd" . "https://redis.io/commands/%s"))
#+END_SRC

#+RESULTS:
: ((rediscmd . https://redis.io/commands/%s) (dwb . http://www.woerterbuchnetz.de/DWB?lemma=%s) (rfc . https://tools.ietf.org/html/%s) (jira . http://jira.adtran.com/browse/%s))
...
Test Link [[rediscmd:object][rediscmd:object]]
...
Just learnt from =starter-kit-python.org= to use a Starter-Kit's function
instead of customize variable =package-selected-packages= and 
call function [[help:package-install-selected-packages][package-install-selected-packages]].
#+BEGIN_EXAMPLE
  (custom-set-variables
...
Starter-Kit function is define in =starter-kit.org= 
with headline [[file:starter-kit.org::*Starter%20kit%20basics][ELPA archive repositories and two packages to install by default]].
#+BEGIN_SRC emacs-lisp :tangle yes
...
- Profiler 
  ...
  - M-x profiler-report
  - M-x profiler-stop
  - M-x profiler-report-write-profile
